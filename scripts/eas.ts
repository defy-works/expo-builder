...
function buildVmScript(
  ...
  // Build optimizations — split into early setup (Gradle) and late setup (config wrapper).
  // Config wrapper must run AFTER bun install because @expo/config needs node_modules
  // to resolve imports in the wrapper, and eas env:pull reads the config to find the project ID.
  ...
  if (optimize) {
    ...
    // Wrap app.config to inject the iOS build optimization plugin.
    // Renames app.config.ts → _app.config.original.ts, then creates a
    // wrapper that imports it, appends the plugin, and re-exports.
    // Must run after bun install so node_modules are available for import resolution.
    lateOptimize.push(
      "",
      "# Inject iOS build optimization plugin via config wrapper",
